...
fun CommandSender.hasPermission(permission: PermissionNode): Boolean {
    val candidates = sequenceOf(permission.node) + permission.parents.asSequence() + permission.aliases.asSequence()
    if (candidates.any { hasPermission(it) }) return true

    if (!hasPermission(LEGACY_ADMIN)) return false
    return candidates.none { candidate ->
        getEffectivePermissions().any { info ->
            info.permission.equals(candidate, ignoreCase = true) && !info.value && info.attachment != null
        }
    }
}
